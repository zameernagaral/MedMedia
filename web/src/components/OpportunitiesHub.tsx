    ...
    <div className="space-y-4 pb-20">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 shadow-xs transition-colors duration-200">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Opportunities Hub</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Verified clinical appointments, research calls, locum gigs, CME & conferences.
            </p>
          </div>
          <span className="text-[11px] font-bold px-2.5 py-1 bg-sky-50 dark:bg-sky-950/60 text-sky-700 dark:text-sky-300 rounded-full border border-sky-200 dark:border-sky-800">
          ...
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition cursor-pointer ${
                  isActive
      ...
      {activeSubTab === 'jobs' && (
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 shadow-xs space-y-3 transition-colors duration-200">
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full text-xs pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500"
                />
              </div>
              ...
              <select
                ...
                className="text-xs bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-500"
              >
          ...
          <div className="space-y-3">
            {filteredJobs.map((job) => (
              <div
                ...
                className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 shadow-xs hover:shadow-md hover:border-sky-300 dark:hover:border-sky-500 transition cursor-pointer flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {researchItems.map((item) => (
              <div key={item.id} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/60 px-2.5 py-1 rounded-full border border-sky-200 dark:border-sky-800">
                    Open Research Call
                  </span>
                  <span className="text-xs text-slate-500 dark:text-slate-400">{item.organizerOrAffiliation}</span>
                </div>

                <h3 className="text-sm font-bold text-slate-900 dark:text-white mt-2">{item.title}</h3>
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{item.subtitle}</p>
                <p className="text-xs text-slate-700 dark:text-slate-300 mt-2 leading-relaxed">{item.description}</p>

                <div className="flex flex-wrap gap-1.5 mt-3">
                  {item.tags.map((t, idx) => (
                    <span key={idx} className="text-[10px] font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-xs text-slate-400 dark:text-slate-500">Co-authorship contract provided</span>
                  <button
                    onClick={() => alert(`Request submitted to Dr. Arvind Ramesh for research collaboration on: ${item.title}`)}
                    className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold rounded-xl transition cursor-pointer"
                  >
                    {item.actionLabel}
                  </button>
                </div>
      ...
      {activeSubTab === 'freelance' && (
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 shadow-xs flex items-center justify-between transition-colors duration-200">
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Locum Tenens & Freelance Shifts</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Find emergency cover, tele-consult shifts, and medical writing contracts.</p>
            </div>
            <button
              onClick={() => alert("Post a locum requirement for your hospital or clinical center.")}
              className="px-3.5 py-1.5 bg-sky-50 dark:bg-sky-950/60 text-sky-700 dark:text-sky-300 border border-sky-200 dark:border-sky-800 font-bold text-xs rounded-xl hover:bg-sky-100 dark:hover:bg-sky-900/60 transition cursor-pointer"
            >
              Post Locum Gig
            </button>
          </div>
          ...
          <div className="space-y-3">
            {freelanceItems.map((item) => (
              <div key={item.id} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 px-2.5 py-1 rounded-full border border-emerald-200 dark:border-emerald-800">
                    Locum Duty
                  </span>
                ...
                <p className="text-xs font-semibold text-sky-600 dark:text-sky-400 mt-1">{item.dateTime}</p>

                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">{item.organizerOrAffiliation}</span>
                  <button
      ...
      {activeSubTab === 'events' && (
        <div className="space-y-4">
          ...
          {/* Event Filter Bar */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 shadow-xs flex items-center justify-between flex-wrap gap-2 transition-colors duration-200">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Filter Events:</span>
              <button
                ...
                className={`text-xs px-3.5 py-1.5 rounded-full font-semibold transition cursor-pointer ${
                  eventNearbyOnly ? 'bg-sky-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                }`}
              >
                📍 Near by you
              </button>
            </div>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Upcoming Medical Summits</span>
          </div>
            ...
            {eventItems.map((item) => (
              <div key={item.id} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-400 dark:text-slate-500">National Accreditation</span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3 p-3.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800/60 rounded-xl text-xs">
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 font-bold block text-[10px] uppercase">Venue:</span>
                    <span className="font-semibold text-slate-800 dark:text-slate-200">{item.locationOrVenue}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 font-bold block text-[10px] uppercase">Time and Date:</span>
                    <span className="font-semibold text-slate-800 dark:text-slate-200">{item.dateTime}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 font-bold block text-[10px] uppercase">Organized By:</span>
                    <span className="font-semibold text-slate-800 dark:text-slate-200">{item.organizerOrAffiliation}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 font-bold block text-[10px] uppercase">Contact / Secretariat:</span>
                ...
                <p className="text-xs text-slate-700 dark:text-slate-300 mt-3 leading-relaxed">{item.description}</p>
      ...
      {activeSubTab === 'courses' && (
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 shadow-xs transition-colors duration-200">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Accredited Continuing Medical Education (CME)</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Earn recognized credit hours and update your clinical protocols.</p>
          </div>

          <div className="space-y-3">
            {courseItems.map((item) => (
              <div key={item.id} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-950/60 px-2.5 py-0.5 rounded-full border border-sky-200 dark:border-sky-800">
                      {item.cmeCredits} CME Credits
                    </span>
                    <span className="text-xs text-slate-400 dark:text-slate-500">• {item.organizerOrAffiliation}</span>
                  </div>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold rounded-xl whitespace-nowrap transition cursor-pointer"
                >
                  {item.actionLabel}
                </button>
              </div>
      ...
      {activeSubTab === 'community' && (
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-teal-800 to-sky-800 text-white p-5 rounded-2xl shadow-xs">
            ...
            <button
              ...
              className="mt-3 px-4 py-1.5 bg-white text-teal-900 text-xs font-bold rounded-xl hover:bg-teal-50 transition cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 shadow-xs flex flex-col justify-between">
                <div>
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white">{grp.name}</h4>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{grp.members}</p>
                  <p className="text-[11px] text-slate-700 dark:text-slate-300 mt-2">Active: {grp.topics}</p>
                </div>
                <button
                  onClick={() => alert(`Joined ${grp.name}!`)}
                  className="mt-3 w-full py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-xs rounded-xl transition cursor-pointer"
                >
