    ...
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-xs transition-colors duration-200">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3">
          ...
          {/* Manager mode toggle button (Admin/dev use) */}
          {onToggleManagerMode && (
            <button
              onClick={onToggleManagerMode}
              className={`hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-bold border transition cursor-pointer ${
                isManagerMode
              ...
              title="Toggle Manager/Admin mode"
            >
              <ShieldAlert className="w-3.5 h-3.5" />
              {isManagerMode ? 'Admin' : 'Admin'}
            </button>
          )}

          {/* Wipe data button (dev only, shown in admin mode) */}
          {onClearAllTestData && isManagerMode && (
            <button
              onClick={onClearAllTestData}
              className="hidden sm:flex items-center gap-1 px-2 py-1.5 rounded-lg text-[11px] font-bold border border-rose-200 dark:border-rose-900 text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-100 transition cursor-pointer"
