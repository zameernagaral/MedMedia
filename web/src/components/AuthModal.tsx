...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden my-6 animate-in fade-in zoom-in-95">
        ...
        <div className="flex border-b border-slate-200 text-xs font-bold bg-slate-50">
          <button
            ...
            className={`flex-1 py-3 text-center transition ${
              mode === 'signup' ? 'text-sky-600 border-b-2 border-sky-600 bg-white' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            New Account (Verify)
          </button>
          <button
            ...
            className={`flex-1 py-3 text-center transition ${
              mode === 'signin' ? 'text-sky-600 border-b-2 border-sky-600 bg-white' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            Sign In / Log In
          </button>
          <button
            ...
            className={`flex-1 py-3 text-center transition ${
              mode === 'recovery' ? 'text-sky-600 border-b-2 border-sky-600 bg-white' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            Recovery
          </button>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className={`p-2 rounded-xl ${selectedRole === 'DOCTOR' ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
                      <Stethoscope className="w-5 h-5" />
                    </div>
                    {selectedRole === 'DOCTOR' && <CheckCircle2 className="w-4 h-4 text-sky-600" />}
                  </div>
                  <div className="mt-2">
                    <h4 className="text-xs font-bold text-slate-900">Doctor Verification</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5">Consultants, Specialists, Surgeons</p>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className={`p-2 rounded-xl ${selectedRole === 'STUDENT' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
                      <GraduationCap className="w-5 h-5" />
                    </div>
                    {selectedRole === 'STUDENT' && <CheckCircle2 className="w-4 h-4 text-emerald-600" />}
                  </div>
                  <div className="mt-2">
                    <h4 className="text-xs font-bold text-slate-900">Student Verification</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5">MBBS, Nursing, B.Pharm, D.Pharm, Lab</p>
                  </div>
                </button>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1 flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-400 font-normal">Used for OTP / Password login</span>
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full text-xs pl-9 pr-3 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500"
                />
              </div>
            </div>
            ...
            <div className="p-4 bg-sky-50/60 border border-sky-200/80 rounded-2xl space-y-3">
              ...
              <div
                ...
                className={`p-3 border-2 border-dashed rounded-xl text-center cursor-pointer transition ${
                  fileUploaded ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-sky-300 bg-white text-slate-600 hover:bg-sky-50'
                }`}
              >
          ...
          {mode === 'signup' && selectedRole === 'STUDENT' && (
            <div className="p-4 bg-emerald-50/60 border border-emerald-200/80 rounded-2xl space-y-3">
              ...
              <div
                onClick={() => setFileUploaded(!fileUploaded)}
                className={`p-3 border-2 border-dashed rounded-xl text-center cursor-pointer transition ${
                  fileUploaded ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-emerald-300 bg-white text-slate-600 hover:bg-emerald-50'
                }`}
              >
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">Eligible for Academic Caduceus Badge</p>
              </div>
            </div>
          )}

          {statusMessage && (
            <p className="text-xs text-center font-semibold text-sky-700 bg-sky-50 py-2 rounded-lg">
              {statusMessage}
            </p>
          )}
          ...
          <div className="pt-2">
            ...
            <button
              ...
              className="w-full py-2.5 px-4 bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 font-bold text-xs rounded-xl shadow-sm transition flex items-center justify-center gap-2"
            >
